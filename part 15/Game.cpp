...
namespace {
    ...
    // glyph -> visual/identity, for reconstructing monsters on load
...
void Game::newGame(PlayerClass cls)
{
    ...
    {
        int order[6]; for (int i = 0; i < 6; ++i) order[i] = i;
        std::shuffle(order, order + 6, m_rng);
        for (int t = 0; t < (int)PotionType::Count; ++t) { m_potionAppearIdx[t] = order[t]; m_potionIdent[t] = false; }
    }
    {
        int order[5]; for (int i = 0; i < 5; ++i) order[i] = i;
        std::shuffle(order, order + 5, m_rng);
...
// ---------------------------------------------------------------------------
//  Save / load — plain text. Reconstruct what we can (item names from
//  category+bonus, monster colour/name from glyph) so we store only numbers.
// ---------------------------------------------------------------------------
void Game::save(const char* path)
{
    std::ofstream f(path);
    if (!f) { flash("Could not save."); return; }
    ...
    f << (m_hasWeapon ? 1 : 0) << ' ' << m_weapon.bonus << ' '
      << (m_hasArmour ? 1 : 0) << ' ' << m_armour.bonus << '\n';
    ...
    f << m_inventory.size() << '\n';
    for (const Item& it : m_inventory)
        f << (int)it.category << ' ' << (int)it.potion << ' ' << (int)it.scroll << ' ' << it.bonus << '\n';
...
{
    std::ifstream f(path);
    if (!f) { flash("No save found."); return false; }

    std::string magic; int version = 0;
    f >> magic >> version;
    if (magic != "RLSAVE") { flash("Bad save file."); return false; }
    ...
    int cls; f >> cls >> m_depth;  m_class = (PlayerClass)cls;
    f >> m_basePower >> m_baseMaxHp >> m_strengthBonus >> m_magicBonus;
    ...
    for (int li = 0; li < levelN; ++li)
    {
        ...
        for (int i = 0; i < mN; ++i)
        {
            ...
            Monster m;
            m.glyph = g.empty() ? '?' : g[0];
            MKind k = kindByGlyph(m.glyph);
            m.color = k.color;  m.name = k.name;  m.venomous = (ven != 0);
            m.x = x; m.y = y; m.hp = hp; m.maxHp = maxHp; m.power = pow;
            m.statusTurns[(int)Status::Poisoned] = ps;
            m.statusTurns[(int)Status::Regen]    = rg;
            m.alive = (alv != 0);
        ...
        for (int i = 0; i < iN; ++i)
        {
            int c, p, s, b, x, y; f >> c >> p >> s >> b >> x >> y;
            Item it = buildItem(c, p, s, b);
            it.x = x; it.y = y;
...
void Game::flash(const char* msg) { SDL_SetWindowTitle(m_window, msg); }
...
{
    for (Monster& m : cur().monsters) if (m.isAlive() && m.x == x && m.y == y) return &m;
    return nullptr;
}
...
std::string Game::potionName(PotionType t) const
{
    ...
    return std::string(COLOURS[m_potionAppearIdx[(int)t]]) + " potion";
}
std::string Game::scrollName(ScrollType t) const
{
    if (m_scrollIdent[(int)t]) return SCROLL_TRUE[(int)t];
    return std::string("scroll labelled \"") + LABELS[m_scrollAppearIdx[(int)t]] + "\"";
}
...
void Game::readScroll(ScrollType t)
{
    ...
    auto zap = [&](Monster& m, int dmg) { m.hp -= dmg; SDL_Log("%s is blasted for %d.", m.name, dmg); if (m.hp <= 0) { m.hp = 0; m.alive = false; SDL_Log("%s is destroyed.", m.name); } };
...
{
    if (it.category == ItemCategory::Weapon) { if (m_hasWeapon) m_inventory.push_back(m_weapon); m_weapon = it; m_hasWeapon = true; SDL_Log("You wield the %s.", it.name); }
    else                                     { if (m_hasArmour) m_inventory.push_back(m_armour); m_armour = it; m_hasArmour = true; SDL_Log("You don the %s.", it.name); }
    recomputeStats();
}
...
void Game::pickUpHere()
{
    ...
    for (size_t i = 0; i < L.items.size(); ++i)
        ...
        { m_inventory.push_back(L.items[i]); SDL_Log("You pick up a %s.", itemLabel(L.items[i]).c_str()); L.items.erase(L.items.begin() + i); break; }
}
...
void Game::updateTitle()
{
    ...
    SDL_snprintf(buf, sizeof(buf),
        "Roguelike - Part 15   |   %s  Depth %d  HP %d/%d POW%d DEF%d%s   (F5 save  F9 load)",
...
{
    int len = (int)SDL_strlen(text), s = (MAP_WIDTH - len) / 2;
    for (int i = 0; i < len; ++i) m_glyphs.drawGlyph(s + i, row, text[i], color);
}
...
void Game::drawClassSelect()
{
    ...
    drawCentred(13, "1)  Fighter  -  tough; sword & chain mail", UI_TEXT);
    drawCentred(15, "2)  Rogue    -  nimble; dagger, leather & a scroll", UI_TEXT);
    drawCentred(17, "3)  Mage     -  frail, but devastating magic", UI_TEXT);
    drawCentred(22, "1 / 2 / 3  begin       F9  load saved game       ESC  quit", UI_HINT);
    ...
    while (running)
    {
        ...
        switch (event.type)
        {
            ...
            case SDL_EVENT_KEY_DOWN:
            {
                ...
                // Playing
                if (sc == SDL_SCANCODE_F5) { save(SAVE_PATH); break; }
