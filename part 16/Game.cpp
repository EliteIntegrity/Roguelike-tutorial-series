...
{
    SDL_SetWindowTitle(m_window, "Roguelike - Part 16: UI & Sound");
    m_sound.init();                  // silent fallback if no audio device
...
void Game::readScroll(ScrollType t)
{
    ...
    log("You read the %s.", scrollName(t).c_str());
    m_sound.zap();
...
void Game::pickUpHere()
{
    ...
    for (size_t i = 0; i < L.items.size(); ++i)
        ...
        { m_inventory.push_back(L.items[i]); m_sound.pickup(); log("You pick up a %s.", itemLabel(L.items[i]).c_str()); L.items.erase(L.items.begin() + i); break; }
}
...
{ int len = (int)SDL_strlen(text), s = (MAP_WIDTH - len) / 2; for (int i = 0; i < len; ++i) m_glyphs.drawGlyph(s + i, row, text[i], color); }
...
void Game::drawHud()
{
    int base = MAP_HEIGHT;       // first HUD row, just below the map
    ...
    char s[160];
    SDL_snprintf(s, sizeof(s), "%s   HP %d/%d   POW %d   DEF %d   Depth %d%s",
                 cn, m_player.hp, m_player.maxHp, m_player.power, m_player.defense, m_depth + 1, status.c_str());
    drawLeft(1, base, s, HUD_STAT);

    const std::vector<std::string>& lines = m_log.lines();
    int slots = HUD_ROWS - 1;
    int count = std::min((int)lines.size(), slots);
    for (int i = 0; i < count; ++i)
    {
        const std::string& ln = lines[lines.size() - count + i];
        drawLeft(1, base + 1 + i, ln.c_str(), (i == count - 1) ? MSG_NEW : MSG_OLD);
    }
}
