...
{
    m_map.generate();
    m_player = Player{};                 // fresh player (clears statuses too)
...
void Game::tickStatuses(Entity& e)
{
    if (e.statusTurns[(int)Status::Poisoned] > 0)
    {
        ...
        if (e.hp <= 0)
        {
            e.hp = 0;
            e.alive = false;
            SDL_Log("%s succumbs to poison.", e.name);
        }
    }
...
{
    monstersAct();
    tickStatuses(m_player);          // poison/regen tick on the player
    updateTitle();
...
void Game::monstersAct()
{
    ...
    for (Monster& m : m_monsters)
    {
        ...
        tickStatuses(m);                 // monsters can be poisoned too
        if (!m.isAlive()) continue;
...
void Game::updateTitle()
{
    ...
    SDL_snprintf(buf, sizeof(buf),
        "Roguelike - Part 10   |   HP: %d/%d   POW:%d DEF:%d%s   (i = inventory)",
        m_player.hp, m_player.maxHp, m_player.power, m_player.defense, status.c_str());
