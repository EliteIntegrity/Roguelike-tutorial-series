...
{
    // One flood fills the whole dungeon with "steps to the player". Every
    // monster then reads its own neighbourhood from the same shared map.
    ...
    for (Monster& m : m_monsters)
    {
        ...
        // Out of sight → wander in the dark, as before.
        ...
        // Wounded monsters flee (seek HIGHER distance); the rest chase (seek
        // LOWER). Same map, read in opposite directions.
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (nx == m_player.x && ny == m_player.y) continue;  // handled by the attack above
...
{
    char buf[96];
    SDL_snprintf(buf, sizeof(buf), "Roguelike - Part 7   |   HP: %d / %d",
                 m_player.hp, m_player.maxHp);
