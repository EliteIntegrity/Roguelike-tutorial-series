...
namespace {
    ...
    const int REGEN_TURNS  = 12;
    const int VENOM_TURNS  = 5;
    const int LIGHTNING_DMG = 6;
    const int FIREBALL_DMG  = 5;
}
...
void Game::newGame()
{
    ...
    // shuffle potion appearances
    {
        ...
        {
            m_potionAppear[t] = COLOURS[order[t]];
            m_potionIdent[t]  = false;
        }
    }
    // shuffle scroll appearances
    {
        int order[NUM_LABELS];
        for (int i = 0; i < NUM_LABELS; ++i) order[i] = i;
        std::shuffle(order, order + NUM_LABELS, m_rng);
        for (int t = 0; t < (int)ScrollType::Count; ++t)
        {
            m_scrollAppear[t] = LABELS[order[t]];
            m_scrollIdent[t]  = false;
        }
    }
...
void Game::quaff(PotionType t)
{
    ...
    switch (t)
    {
        ...
        case PotionType::Poison:
            m_player.addStatus(Status::Poisoned, POISON_TURNS);
            SDL_Log("Your veins burn — poison!");
            break;
        case PotionType::Strength:
            m_strengthBonus += 1; recomputeStats();
            SDL_Log("Your muscles swell. (power %d)", m_player.power);
            break;
        case PotionType::Regen:
            m_player.addStatus(Status::Regen, REGEN_TURNS);
...
void Game::playerAct(int dx, int dy)
{
    ...
    else if (m_player.tryMove(dx, dy, cur().map))
    {
...
{
    std::string status;
    if (m_player.statusTurns[(int)Status::Poisoned] > 0)
        status += "  POISON(" + std::to_string(m_player.statusTurns[(int)Status::Poisoned]) + ")";
    if (m_player.statusTurns[(int)Status::Regen] > 0)
        status += "  REGEN(" + std::to_string(m_player.statusTurns[(int)Status::Regen]) + ")";

    char buf[192];
    SDL_snprintf(buf, sizeof(buf),
        "Roguelike - Part 13   |   Depth %d   |   HP %d/%d  POW%d DEF%d%s   (i inv, > stairs)",
        m_depth + 1, m_player.hp, m_player.maxHp, m_player.power, m_player.defense, status.c_str());
...
void Game::drawDeathScreen()
{
    ...
    const int x0 = (MAP_WIDTH - boxW) / 2, y0 = (MAP_HEIGHT - boxH) / 2;
