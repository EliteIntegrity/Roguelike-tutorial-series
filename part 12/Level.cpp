...
{
    // Alternate dungeon styles by depth: even floors are BSP rooms-and-
    // corridors, odd floors are cellular-automata caves. Either way the map
    // ends up with a rooms[] list, so everything below is identical.
    ...
    // Down-stairs in the last room — far from where the player will enter (room 0).
    const Room& last = map.rooms[n - 1];
    stairsX = last.centreX();
    stairsY = last.centreY();

    // Depth scaling: more monsters the deeper you go, each toughened a little.
    ...
    for (int i = 1; i < n && (int)monsters.size() < monsterCount; ++i)
    {
        ...
        m.power = k.power + depth / 2;            // and hit harder
        m.name = k.name;
        m.venomous = k.venom;
        monsters.push_back(m);
    }

    // A handful of items scattered on random floor tiles.
    int itemCount = 7;
